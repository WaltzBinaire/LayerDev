...
void main()
{
    ...
    vec4 color = texture( u_imageTex, texCoordVarying );
    brightnessAdjust(color, u_brightness);     
    contrastAdjust  (color, u_contrast); 
    
    outputColor = saturationMatrix(u_saturation) * color;
    outputColor = vibrance(outputColor, u_vibrance);
    outputColor = shiftHue(outputColor.rgb, u_hue);
}